...
public class SaltDatabase extends Database {
    ...
    public List<MTProtoScheme.future_salt> getSalts(long authId, int limit) {
        ...
        try (ResultSet resultSet = query(null, "auth_id = " + authId + " AND valid_until > " +
            ...
            while (resultSet.next()) {
                ...
                futureSalt.valid_since = resultSet.getInt(4);
                futureSalt.valid_until = resultSet.getInt(5);
    ...
    public void setSalts(long authId, List<MTProtoScheme.future_salt> salts) {
        for (MTProtoScheme.future_salt futureSalt : salts) {
            ...
            columnValue.put("valid_since", futureSalt.valid_since);
            columnValue.put("valid_until", futureSalt.valid_until);
            try {
                insert(columnValue);
            } catch (SQLException e) {
                e.printStackTrace();
