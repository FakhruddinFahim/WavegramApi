    ...
    public AuthKey getAuthKey(long authId) {
        ...
        try (ResultSet resultSet = executeQuery("SELECT * FROM " + getTableName() + " WHERE auth_id = " + authId)) {
            if (resultSet.next()) {
                ...
                authKey.expireAt = resultSet.getInt("expire_date");
                authKey.setType(authKey.expireAt > 0 ? AuthKey.Type.TEMP_AUTH_KEY : AuthKey.Type.PERM_AUTH_KEY);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void setAuthKey(AuthKey authKey) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO " + getTableName() +
                    " (auth_id, auth_key, type, expire_date) VALUES("
                    + authKey.getAuthKeyId() + ", ?, ?, " + authKey.expireAt + ")");
            preparedStatement.setBlob(1, new ByteArrayInputStream(authKey.getAuthKey()));
            preparedStatement.setInt(2, authKey.type == AuthKey.Type.PERM_AUTH_KEY ? 0 : 1);
