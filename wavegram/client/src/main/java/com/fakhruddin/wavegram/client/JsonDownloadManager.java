    ...
    try {
      if (file.exists()) {
        ...
        if (jsonObject.has(String.valueOf(fileId))) {
          ...
          downloadFile.inputFileLocation = (ApiScheme.InputFileLocationType) TLContext.read(new TLInputStream(Base64.getDecoder().decode(jsonFileInfo.getString("inputFileLocation"))));
          if (jsonFileInfo.has("downloadedParts")) {
    ...
    try {
      if (file.exists()) {
        ...
        for (String key : jsonObject.keySet()) {
          ...
          downloadFile.inputFileLocation = (ApiScheme.InputFileLocationType) ApiContext.read(new TLInputStream(Base64.getDecoder().decode(jsonFileInfo.getString("inputFileLocation"))));
